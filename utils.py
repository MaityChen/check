...
class config_get(object):
    ...
    def move_configuration_file_old():
        print("移动配置文件")
        os.system("cp /ql/repo/MaityChen_check/check.sample.toml /ql/config/check.toml")

    @staticmethod
    def move_configuration_file_new():
        print("移动配置文件")
        os.system("cp /ql/data/repo/MaityChen_check/check.sample.toml /ql/data/config/check.toml")
...
def change_cron_new(cron_file_path="/ql/data/db/database.sqlite", repositories="MaityChen_check"):
    print("尝试修改定时时间")
...
def change_cron_old(cron_file_path="/ql/db/crontab.db", repositories="MaityChen_check"):
    print("尝试修改定时时间")
